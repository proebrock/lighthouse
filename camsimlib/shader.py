...
class Shader:
    ...
    def run(self, ray_tracer):
        ...
        if self.lighting_mode == 'point':
            ...
            # We assume that self.light_vector is a vector describing the light direction
            # of a parallel light source
            # lightvec is a unit vector in the direction of the light source
            lightvec = -self.light_vector
            lightvec /= np.linalg.norm(lightvec)
            # Dot product of vertex_normals and lightvecs; if angle between
            # those is 0°, the intensity is 1; the intensity decreases up
            # to an angle of 90° where it is 0
            vertex_intensities = np.sum(vertex_normals * lightvec, axis=2)
        else:
